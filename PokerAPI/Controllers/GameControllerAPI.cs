    ...
    namespace PokerAPI.Controllers
    {
        ...
        public class GameControllerAPI : ControllerBase
        {
            ...
            public GameControllerAPI(GameController game, IHubContext<PokerHub> hub)
            {
                _game = game;
                _hub = hub;
            ...
            private async Task BroadcastGameState()
            {
                var state = new
                {
                    ...
                    {
                        seatIndex = kv.Key.SeatIndex,
                        name = kv.Key.Name,
                        chipStack = kv.Key.ChipStack,
                        state = kv.Value.State.ToString(),
                        currentBet = kv.Value.CurrentBet,
                        hand = kv.Value.Hand.Select(c => $"{c.Rank} of {c.Suit}")
                    })
                };

                await _hub.Clients.All.SendAsync("ReceiveGameState", state);
            }
