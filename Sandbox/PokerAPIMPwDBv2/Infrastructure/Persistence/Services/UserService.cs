...
public class UserService : IUserService
{
    ...
    public async Task<ServiceResult> SoftDeleteUserAsync(Guid userId)
    {
        ...
        user.isDeleted = true;
        await _db.SaveChangesAsync();
        return ServiceResult.Success("User soft deleted");
    }

    public async Task<ServiceResult> DepositAsync(Guid userId, int amount)
    {
        if (amount <= 0) return ServiceResult.Fail("Deposit amount must be positive");
        ...
        return ServiceResult.Success($"Successfully deposited {amount}. New balance: {user.Balance}");
    }
}
